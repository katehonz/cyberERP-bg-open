defmodule CyberCore.Repo.Migrations.CreatePurchaseOrderLines do
  use Ecto.Migration
  ...
  def change do
    create table(:purchase_order_lines) do
      ...
      add :quantity_ordered, :decimal, precision: 15, scale: 4, null: false
      add :quantity_received, :decimal, precision: 15, scale: 4, default: 0, null: false
    ...
    create index(:purchase_order_lines, [:tenant_id])
    create index(:purchase_order_lines, [:purchase_order_id])
    create index(:purchase_order_lines, [:product_id])
  end
end
