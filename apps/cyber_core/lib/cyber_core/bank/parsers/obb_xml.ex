defmodule CyberCore.Bank.Parsers.OBBXML do
  @moduledoc """
  Парсер за XML файлове от ОББ (Обединена Българска Банка).

  ОББ използва собствен XML формат.
  """
  ...
  defp parse_xml(xml_content) do
    # OBB използва custom XML формат
    # Този парсер е опростена версия и трябва да се адаптира
    # към действителния формат на ОББ
    ...
    error ->
      {:error, "Failed to parse OBB XML: #{Exception.message(error)}"}
  end
